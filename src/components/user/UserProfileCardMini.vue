<template>
  <div class="user-card-mini">
    <div class="user-header">
      ...
      <div class="coins">
        硬币: <span>{{ user.coin }}</span> &nbsp; B币: <span>{{ user.bcoin }}</span>
      </div>
    </div>
    <div class="level-row">
      <span class="level">LV{{ user.level }}</span>
      <span class="exp"
        >当前成长{{ user.exp }}，距离升级Lv.{{ user.level + 1 }} 还需{{
          user.nextExp - user.exp
        }}</span
      >
    </div>
    <div class="stats-row">
      ...
      <div>
        <div class="stat-num">{{ user.dynamic }}</div>
    ...
    <div class="menu-list">
      <router-link to="/user/profile" class="menu-item nav-link"
        ><i class="bi bi-person me-2"></i> 个人中心</router-link
      >
      <router-link to="/upload" class="menu-item nav-link"
        ><i class="bi bi-file-earmark-play me-2"></i> 投稿管理</router-link
      >
      <router-link to="/user/recommend" class="menu-item nav-link"
        ><i class="bi bi-file-earmark-play me-2"></i> 推荐服务</router-link
      >
    </div>
